...
- Dev builds default to `0.5.1-SNAPSHOT`.
- Release builds use the pushed tag name (for example `v0.5.1` becomes version `0.5.1`).
...
```bash
DWARVENPICK_VERSION=0.5.1 ./gradlew :backend:app:bootJar
```
