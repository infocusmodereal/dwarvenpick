...
PostgreSQL is also the coordination boundary for atomic per-actor, global, and per-connection query concurrency admission
across replicas; do not run multiple backend replicas against H2.
